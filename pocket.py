...
def bd_last_check(sqlite_path):
    db = sqlite3.connect(sqlite_path)
    cursor = db.cursor()
    cursor.execute(create_db_query)
    cursor.execute('''SELECT scan_start_time
                      FROM subscriber
                      ORDER BY scan_start_time
                      DESC LIMIT 1''')
    last_check = cursor.fetchone()
...
def pocket(outlines, key, token):
    outlines_len = len(outlines)
    new_count = 0
    sys.stdout.write('Sending to pocket:')
    sys.stdout.flush()
    for outline in outlines:
        data = {"url": outline['url'], "title": outline['title'],
                "consumer_key": key,
                "access_token": token}
        try:
            pass
            req = requests.post(url, headers=headers, data=json.dumps(data), timeout=5)
            if req.status_code != 200:
                print 'Error: {}'.format(req.headers.get('x-error'))
                if req.headers.get('x-limit-user-remaining') == '0':
                    sleep = int(req.headers.get('x-limit-user-reset'))
                    if sleep:
                        print 'Wait for {} sec'.format(req.headers.get('x-limit-user-reset'))
                        time.sleep(sleep)
            #print req.headers
        except Exception as error:
            print error
            ...
            new_count += 1
        percents = '{}%'.format(int((outlines.index(outline)+1) / (outlines_len/100.0)))
        sys.stdout.write(percents)
        sys.stdout.flush()
        sys.stdout.write('\b' * len(percents))
    return new_count
...
def get_outlines(doc):
    ...
    return feeds_count, outlines, errors
...
def process(filename):
    ...
    sqlite_path = config.get('subscriber', 'sqlite_path')
    opml_path = config.get('subscriber', 'opml_path')
    key = config.get('subscriber', 'pocket_consumer_key')
    token = config.get('subscriber', 'pocket_access_token')

    doc = minidom.parse(opml_path)
    last_check = bd_last_check(sqlite_path)
    if (last_check and date_created_oml(doc) > datetime.fromtimestamp(last_check[0])) or not last_check:
        scan_start_time = time.time()
        feeds_count, outlines, errors = get_outlines(doc)
        new_count = pocket(outlines, key, token)
        if errors['errors']:
            status_extras = json.dumps(errors)
        else:
            status_extras = ''
        if new_count == 0:
            status_string = 'failed'
        elif new_count == feeds_count:
            status_string = 'done'
        else:
            status_string = 'done with errors'

        write_database(sqlite_path, scan_start_time,
                       new_count, feeds_count, status_string, status_extras)
        print 'new_count: {}; feeds_count:{}; status_string:{};'.format(new_count, feeds_count, status_string)
    else:
        print 'now new feeds in opml: {}'.format(opml_path)
